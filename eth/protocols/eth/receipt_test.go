...
func TestReceiptList(t *testing.T) {
	for i, test := range receiptsTests {
		...
		// encode block body from types object (Body.EncodeRLP has pointer receiver).
		blockBody := &types.Body{Transactions: test.txs}
		canonBody, err := rlp.EncodeToBytes(blockBody)
		if err != nil {
			t.Fatalf("test[%d]: encode block body: %v", i, err)
		}
