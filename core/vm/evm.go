...
type BlockContext struct {
	...
	// Header optionally carries the block header for libevm precompiles; it may be nil.
	Header *types.Header
}
...
type EVM struct {
	...
	executionInvalidated error // set by libevm precompiles via [EVM.InvalidateExecution]
}
...
func NewEVM(blockCtx BlockContext, statedb StateDB, chainConfig *params.ChainConfig, config Config) *EVM {
	blockCtx, txCtx, statedb, chainConfig, config := overrideNewEVMArgs(blockCtx, TxContext{}, statedb, chainConfig, config)
	evm := &EVM{
		Context:     blockCtx,
		TxContext:   txCtx,
		StateDB:     statedb,
...
func (evm *EVM) SetTxContext(txCtx TxContext) {
	...
	txCtx, statedb := evm.overrideEVMResetArgs(txCtx, evm.StateDB)
	evm.TxContext = txCtx
	evm.StateDB = statedb
	evm.executionInvalidated = nil
}
...
func (evm *EVM) Call(caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if isPrecompile {
		args := &evmCallArgs{evm: evm, callType: Call, caller: caller, addr: addr, input: input, gasRemaining: gas, value: value}
		ret, gas, err = runPrecompiledOrStateful(evm, args, p, input)
	} else {
...
func (evm *EVM) CallCode(caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if p, isPrecompile := evm.precompile(addr); isPrecompile {
		args := &evmCallArgs{evm: evm, callType: CallCode, caller: caller, addr: addr, input: input, gasRemaining: gas, value: value}
		ret, gas, err = runPrecompiledOrStateful(evm, args, p, input)
	} else {
		// Initialise a new contract and set the code that is to be used by the EVM.
		// The contract is a scoped environment for this execution context only.
...
func (evm *EVM) DelegateCall(originCaller common.Address, caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// It is allowed to call precompiles, even via delegatecall
	if p, isPrecompile := evm.precompile(addr); isPrecompile {
		args := &evmCallArgs{evm: evm, callType: DelegateCall, caller: caller, addr: addr, input: input, gasRemaining: gas, value: value, originCaller: originCaller}
		ret, gas, err = runPrecompiledOrStateful(evm, args, p, input)
	} else {
...
func (evm *EVM) StaticCall(caller common.Address, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	if p, isPrecompile := evm.precompile(addr); isPrecompile {
		args := &evmCallArgs{evm: evm, callType: StaticCall, caller: caller, addr: addr, input: input, gasRemaining: gas, value: nil}
		ret, gas, err = runPrecompiledOrStateful(evm, args, p, input)
	} else {
		// Initialise a new contract and set the code that is to be used by the EVM.
		// The contract is a scoped environment for this execution context only.
